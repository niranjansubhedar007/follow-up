  ...
  useEffect(() => {
    const loadFormattedTasks = async () => {
      ...
      try {
        ...
        if (response.status === 200) {
          const formattedTasks = response.data.tasks
            .filter((task) => new Date(task.deadlineDate) >= new Date()) // Filter out tasks with deadlineDate less than current date
            .map((task) => ({
              ...task,
                ...
                {tasks.length > 0 ? (
                  currentTasks.map((task, index) => (
                    <tr key={task._id}>
                      ...
                      <td className="px-2 py-1 whitespace-nowrap border border-orange-400 text-center">{task.title}</td>
                      <td className="px-6 py-1 whitespace-nowrap border border-orange-400 text-center">Pending</td>
                      ...
                      <td className="px-5 py-1 whitespace-nowrap border border-orange-400">
                        ...
                        <FontAwesomeIcon
                          icon={faEye}
                          className="text-blue-500 hover:underline -mr-10 cursor-pointer pl-7 my-1.5  text-lg text-center"
